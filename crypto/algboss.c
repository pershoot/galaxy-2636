...
static int cryptomgr_test(void *data)
{
    ...
    // <B2B Security,
    // 2011.07.04   SKIP Self Test for NVIDIA Crypto, param->alg="disabled_*"
    const char *nvidia_prefix = "disabled";
    //printk("cryptomgr_test()::param->driver=%s, param->alg=%s\n", param->driver, param->alg);
    if (param->alg > strlen(nvidia_prefix)) {
        if (!memcmp(param->alg, nvidia_prefix, strlen(nvidia_prefix)))
            goto skiptest;
    }
