...
bool in_fips_err() {return (IN_FIPS_ERROR == FIPS_ERR);}
void set_in_fips_err() {IN_FIPS_ERROR = FIPS_ERR;}
...
test_done:
	if (fips_enabled && rc) 
	// change@wtl.kSingh - FIPS mode self test failure error - starts
	{
		//panic("%s: %s alg self test failed in fips mode!\n", driver, alg);
		printk(KERN_INFO "FIPS: %s: %s alg self test failed in fips mode!\n",
		       driver, alg);
                IN_FIPS_ERROR = FIPS_ERR;
		return rc;
	}
	// change@wtl.kSingh - FIPS mode self test failure error - ends
...
notest:
// disable unnecessary logs, by bumhan.kim
	//printk(KERN_INFO "FIPS: No test for %s (%s)\n", alg, driver);
	return 0;
non_fips_alg:
// change@wtl.kSingh - we allow non-FIPS algorithms - starts
// disable unnecessary logs, by bumhan.kim
#if 0
	if (!rc)
		printk(KERN_INFO "FIPS: self-tests for non-FIPS %s (%s) passed\n",
		       driver, alg);
#endif
    if (rc)
        printk(KERN_INFO "FIPS: self-tests for non-FIPS %s (%s) failed\n", alg, driver);
	return rc;
// change@wtl.kSingh - we allow non-FIPS algorithms - ends
}
