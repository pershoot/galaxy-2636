/*
 * tegra_soc_wm8994_p3.c  --  SoC audio for Samsung P3
 *
 * (c) 2011 Samsung Electronics, Inc.
 *
...
static struct wm8994_setup_data tegra_wm8994_setup = {
	/*
...
static int __init tegra_init(void)
{
	...
	/* enable boot-up with broken codec */
	if (!tegra_snd_devdata.card->codec->drvdata) {
		pr_err("%s: need to check wm8994 codec\n", __func__);
		snd_soc_free_pcms(&tegra_snd_devdata);
	}
