...
int imm_vibrator_chip_enable(void)
{
	...
	gpio_direction_output(GPIO_MOTOR_EN2, 1);
...
int imm_vibrator_chip_disable(void)
{
	...
	gpio_direction_output(GPIO_MOTOR_EN2, 0);
...
static int __init p3_init_vibrator(void)
{
	...
	tegra_gpio_enable(GPIO_MOTOR_EN2);
	ret = gpio_request(GPIO_MOTOR_EN2, "vibrator-en2");
	if (ret < 0)
		goto err_gpio_req2;
...
err_to_dev_reg:
	...
	gpio_free(GPIO_MOTOR_EN2);
err_gpio_req2:
	tegra_gpio_disable(GPIO_MOTOR_EN2);
