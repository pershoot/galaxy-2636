...
static struct tegra_suspend_platform_data p3_suspend_data = {
	...
	.corereq_high	= false,
	.sysclkreq_high	= true,
	.wake_enb	= TEGRA_WAKE_GPIO_PS4 | TEGRA_WAKE_GPIO_PJ7 |
		TEGRA_WAKE_GPIO_PS5 |TEGRA_WAKE_GPIO_PQ6 |
		TEGRA_WAKE_GPIO_PW2 | TEGRA_WAKE_GPIO_PQ7 |
		TEGRA_WAKE_RTC_ALARM | TEGRA_WAKE_GPIO_PY6 |
		TEGRA_WAKE_GPIO_PI5,
	.wake_high	= TEGRA_WAKE_GPIO_PS4,
	.wake_low	= TEGRA_WAKE_GPIO_PQ7 | TEGRA_WAKE_GPIO_PY6 | TEGRA_WAKE_GPIO_PJ7,
	.wake_any	= TEGRA_WAKE_GPIO_PW2 | TEGRA_WAKE_RTC_ALARM |
				    TEGRA_WAKE_GPIO_PS5 |TEGRA_WAKE_GPIO_PQ6 | 
				    TEGRA_WAKE_GPIO_PI5,
};

static struct tegra_suspend_platform_data p3_suspend_data_rev05 = {
	/*
	 * Check power on time and crystal oscillator start time
	 * for appropriate settings.
	 */
	...
	.wake_any	= TEGRA_WAKE_GPIO_PB6 | TEGRA_WAKE_RTC_ALARM |
				TEGRA_WAKE_GPIO_PJ7 | TEGRA_WAKE_GPIO_PS5 |
				TEGRA_WAKE_GPIO_PQ6,
};
...
int __init p3_regulator_init(void)
{
	...
	if (charging_mode_from_boot) {		
		if (system_rev > 0x0A) {
			...
			p3_suspend_data_rev05.wake_enb = (TEGRA_WAKE_GPIO_PS4 |
						TEGRA_WAKE_GPIO_PQ7 |
						TEGRA_WAKE_RTC_ALARM);
			p3_suspend_data_rev05.wake_low = TEGRA_WAKE_GPIO_PQ7;
		}
	}
#endif
        ...
        if (system_rev > 0x0A)
            ...
            tegra_init_suspend(&p3_suspend_data_rev05);
