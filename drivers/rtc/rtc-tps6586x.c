...
#ifdef CONFIG_MACH_SAMSUNG_VARIATION_TEGRA
	err = tps6586x_read(tps_dev, RTC_CTRL, &rtc_ctrl);
	if (err < 0) {
		dev_err(&pdev->dev, "unable to read RTC CTRL register\n");
		goto fail;
	}
	if(!(rtc_ctrl&POR_RESET_N)) {
		/* set default time to pdata->default_year */
		default_time.tm_year = (pdata->default_year) - 1900;
...
#ifdef CONFIG_MACH_SAMSUNG_VARIATION_TEGRA
	if(reset_flag) {
		err = tps6586x_rtc_set_time(&pdev->dev, &default_time);
		if (err < 0) {
			dev_err(&pdev->dev, "unable to set default time to default year\n");
			goto fail;
		}
		reset_flag = 0;
	}
