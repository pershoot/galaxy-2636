...
int mms100_ISC_download_binary_data(bool touch_id)
{
	...
	filp = filp_open(MELFAS_FW3, O_RDONLY, 0);
	if (IS_ERR(filp)) {
	...
	filp = filp_open(MELFAS_FW4, O_RDONLY, 0);
	if (IS_ERR(filp)) {
		pr_err("file open error:%d\n", (s32)filp);
		return -1;
	}
...
//	spin_lock(&lock);
    ...
    if(MELFAS_CORE_FIRWMARE_UPDATE_ENABLE) 
        {
            version_info = mms100_ISC_read_data(MCSTS_FIRMWARE_VER_REG_CORE);
    ...
    if(MELFAS_PRIVATE_CONFIGURATION_UPDATE_ENABLE)
        {
            version_info = mms100_ISC_read_data(MCSTS_FIRMWARE_VER_REG_PRIVATE_CUSTOM);
    ...
    if(MELFAS_PUBLIC_CONFIGURATION_UPDATE_ENABLE)
        {
            version_info = mms100_ISC_read_data(MCSTS_FIRMWARE_VER_REG_PUBLIC_CUSTOM);
    ...
    for(i=0;i<3;i++)
    {
        ...
        {
            if(i<2) // core, private custom
                nRet = mms100_ISC_download( (const UINT8*) fw_data1, (const UINT16)fw1_size, (const INT8)i);
            else 
				nRet = mms100_ISC_download( (const UINT8*) fw_data2, (const UINT16)fw2_size, (const INT8)i);
            if (nRet)
                goto fw_error;
#if MELFAS_ISC_2CHIP_DOWNLOAD_ENABLE
            nRet = mms100_ISC_Slave_download((const INT8) i); 
            if (nRet)
                goto fw_error;
#endif
       }
    ...
    for(i=0;i<3;i++)
    {
        if(dl_enable_bit & (1<<i))
        {
            ...
            {
				if (touch_id)
					nRet = mms100_ISC_download( (const UINT8*) MELFAS_binary_3, (const UINT16)MELFAS_binary_nLength_3, (const INT8)i);
				else
