...
void INC_EXTENSION_010(ST_FIB_INFO* pFibInfo)
{
	...
	INC_MSG_PRINTF(1, "\r\n UTC %.4d-%.2d-%.2d : %.2d.%.2d.%.2d[%dms]",
		pList->stDate.usYear, pList->stDate.ucMonth, pList->stDate.ucDay,
		pList->stDate.ucHour, pList->stDate.ucMinutes, pList->stDate.ucSeconds,
...
void INC_EXTENSION_013(ST_FIB_INFO* pFibInfo)
{
	...
	ST_USERAPP_GROUP_INFO	stUserApp;
...
INC_UINT8 INC_ADD_USERAPP_TYPE(INC_UINT32 ulSID, ST_FICDB_SERVICE_COMPONENT* pSvcComponent, INC_INT16 nCnt, ST_USERAPP_GROUP_INFO* pstData)
{
	...
	for(nLoop = 0; nLoop < nCnt; nLoop++, pSvcComponent++){
		if(pSvcComponent->ulSid == ulSID && pSvcComponent->ucIsAppData == INC_ERROR){
			pSvcComponent->stUApp = *pstData;
			pSvcComponent->ucIsAppData = INC_SUCCESS;
#ifdef INC_FIC_DEBUG_MESSAGE
			INC_MSG_PRINTF(1, "\r\n TYPE[0/13]User Application Type file SID:0x%.8X ", ulSID);
#endif		
			return INC_SUCCESS;
		}
	}
...
void INC_FIND_USERAPP_TYPE(INC_UINT32 ulSID, ST_USERAPP_GROUP_INFO* pstData)
{
	...
	INC_ADD_USERAPP_TYPE(ulSID, pSvcComponent, pStreamInfo->nPrimaryCnt, pstData);

	pSvcComponent = pStreamInfo->astSecondary;
	INC_ADD_USERAPP_TYPE(ulSID, pSvcComponent, pStreamInfo->nSecondaryCnt, pstData);

	pStreamInfo = &pList->stDATA;
	pSvcComponent = pStreamInfo->astPrimary;
	if(INC_ADD_USERAPP_TYPE(ulSID, pSvcComponent, pStreamInfo->nPrimaryCnt, pstData) == INC_SUCCESS)
		pList->nUserAppCnt++;

	pSvcComponent = pStreamInfo->astSecondary;
	if(INC_ADD_USERAPP_TYPE(ulSID, pSvcComponent, pStreamInfo->nSecondaryCnt, pstData) == INC_SUCCESS)
		pList->nUserAppCnt++;
}
...
void INC_EXTENSION_110(ST_FIB_INFO* pFibInfo)
{
	...
	INC_UINT8		ucLoop;
	INC_UINT16		uiEId;
	INC_UINT8		acBuff[MAX_LABEL_CHAR];
...
void INC_EXTENSION_115(ST_FIB_INFO* pFibInfo)
{
	...
	ST_TYPE_1*		pType;
	INC_UINT32		ulSId;
	INC_UINT8		ucIndex, acBuff[MAX_LABEL_CHAR];
...
INC_UINT8 INC_FICPARSING(INC_UINT8 ucI2CID, INC_UINT8* pucFicBuff, INC_INT32 uFicLength, ST_SIMPLE_FIC bSimpleFIC)
{
	...
	if(pList->nLabelCnt && pList->nSubChannelCnt && pList->nEmsembleLabelFlag) {

#ifdef USER_APPLICATION_TYPE
		if(pList->nLabelCnt == pList->nSubChannelCnt && pList->nPacketCnt == pList->nPacketModeCnt &&
			pList->nUserAppCnt == pList->nPacketCnt && bSimpleFIC == SIMPLE_FIC_DISABLE)
			return INC_SUCCESS;
#else
		if(pList->nLabelCnt == pList->nSubChannelCnt && pList->nPacketCnt == pList->nPacketModeCnt &&
...
void INC_LABEL_FILTER(INC_UINT8* pData, INC_INT16 nSize)
{
	...
	for(nLoop = nSize-1; nLoop >= 0; nLoop--){
		if(pData[nLoop] == 0x20) pData[nLoop] = INC_NULL;
		else{
			//if(nLoop == nSize-1) 
...
void INC_ADD_SERVICE_LABEL(ST_STREAM_INFO* pStreamInfo, INC_UINT32 ulSID, INC_UINT8* pcLabel)
{
	...
	for(nLoop = 0 ; nLoop < pStreamInfo->nPrimaryCnt; nLoop++, pSvcComponent++){
		if(pSvcComponent->ulSid == ulSID && !pSvcComponent->ucIsLable){
			...
			pList->nLabelCnt++;
#ifdef INC_FIC_DEBUG_MESSAGE
			INC_MSG_PRINTF(1, "\r\n TYPE[1/11] SID[0x%.8X] Service label[ %s ] ", ulSID, pSvcComponent->aucLabels);
#endif
		}
	}
...
#ifdef INC_FIC_DEBUG_MESSAGE
			INC_MSG_PRINTF(1, "\r\nSID[0x%.8X] Service Component label[ %s ] ", ulSID, pSvcComponent->aucComponentLabels);
#endif	
		}
	}
...
#ifdef INC_FIC_DEBUG_MESSAGE
			INC_MSG_PRINTF(1, "\r\n TYPE[0/1] SUB CH ID %x", ucSubChID);
#endif
		}
	}
...
INC_UINT8 INC_ADD_ENSEMBLE_NAME(INC_UINT16 unEnID, INC_UINT8* pcLabel)
{
	ST_FICDB_LIST*	pList;
	pList = INC_GET_FICDB_LIST();
	if(pList->ucIsEnsembleName == INC_SUCCESS) return INC_ERROR;
