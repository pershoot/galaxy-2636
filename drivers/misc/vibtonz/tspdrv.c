...
late_initcall(vibe_init_module);
module_exit(vibe_cleanup_module);
...
static ssize_t write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    */
    ...
    {
        DbgOut((KERN_ERR "tspdrv: invalid write buffer size(%d).\n", count));
        return 0;
    }
