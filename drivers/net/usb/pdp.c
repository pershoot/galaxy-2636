...
{
	struct pdp_priv *priv = container_of(work, struct pdp_priv,
			tx_work);
...
int pdp_netif_rx(struct net_device *parent_ndev, struct sk_buff *skb,
		 int svnet_ch, const __be16 protocol)
{
...
#ifdef CONFIG_BRIDGE
	struct ethhdr *ehdr;
	const char source[ETH_ALEN] = {0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC};
	const char dest[ETH_ALEN] = {0x12, 0x00, 0x00, 0x00, 0x00, 0x00};
#endif

	if (pdp->pdp_devs[index] == NULL) {
		dev_err(&parent_ndev->dev, "err - pdp%d unactivated.\n", index);
		return -ENODEV;
	}
