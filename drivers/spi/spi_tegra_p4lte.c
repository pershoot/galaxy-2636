...
#ifdef CONFIG_MACH_SAMSUNG_P4LTE
	tspi->rx_complete = false;
	tspi->tx_complete = false;
#else
	tspi->rx_complete = 0;
	tspi->tx_complete = 0;
#endif
...
#ifdef CONFIG_MACH_SAMSUNG_P4LTE
	tspi->tx_complete = true;

	if (tspi->cur->rx_buf == NULL || tspi->rx_complete)
...
#ifdef CONFIG_MACH_SAMSUNG_P4LTE
	tspi->rx_complete = true;

	if(tspi->cur->tx_buf == NULL || tspi->tx_complete)
		complete_operation(req);
#else
	(tspi->rx_complete)++;
