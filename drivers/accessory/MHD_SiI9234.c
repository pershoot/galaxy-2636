...
static void sii9234_initial_registers_set(void);
...
void MHD_OUT_EN(void)
{
	...
	printk("[HDMI]MHD_OUT_EN INT register value is: 0x%02x \n", int_stat);
	state = ReadIndexedRegister(INDEXED_PAGE_0, 0x81);
	printk("[HDMI]MHD_OUT_EN register 0x81 value is: 0x%02x\n", state);
	
	if((state & 0x02) && (int_stat &0x01))
	{	
		printk("[HDMI]MHD_OUT_EN :: enable output\n");		
...
static void sii9234_register_init(void)
{
	...
	if(is_JIG_ON_high()) {
		printk(KERN_INFO	"[HDMI] MHL init for factory mode\n");
		I2C_WriteByte(0x72, 0xA3, 0xFF);	// SIMG: Output Swing  default EB
		I2C_WriteByte(0x72, 0xA6, 0x0C);	// SIMG: Swing Offset
	} else {
		printk(KERN_INFO	"[HDMI] MHL init for user mode\n");
	I2C_WriteByte(0x72, 0xA3, 0xC0);	// SIMG: Output Swing  default EB
